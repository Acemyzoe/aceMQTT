...
// 设置客户端ID
void CClient4MQTT::setClientID(const std::string &strClientID)
{
    m_strClientID = strClientID;
}

// 设置服务器地址
void CClient4MQTT::setServerAddress(const std::string &strAddress)
{
    m_strServerAddress = strAddress;
}

// 设置订阅的主题
void CClient4MQTT::cleanSubTopics()
{
    m_vecSubTopics.clear();
}

void CClient4MQTT::addSubTopic(const std::string &strTopic)
{
    m_vecSubTopics.push_back(strTopic);
}

// 设置MQTT消息到达回调
void CClient4MQTT::setMessageArrivedCB(CClient4MQTT::CALLBACK_MessageArrived cb)
{
    m_cbMessageArrived = cb;
}

// 启动客户端
bool CClient4MQTT::start()
{
    return m_thread.start();
}

// 停止客户端
void CClient4MQTT::stop()
{
    m_thread.quit();
    m_thread.wait();
}
...
// 发布消息
void CClient4MQTT::publish(const std::string &strTopic, const std::string &strMessage)
{
    m_thread.emitEvent("publish", new STopicMessage(strTopic, strMessage));
}

// 发送定时器脉冲事件
void CClient4MQTT::emitTimerEvent()
{
...
void CClient4MQTT::__threadStart(void *pData)
{
    OUTPUT("@CClient4MQTT::__threadStart call");
}

void CClient4MQTT::__threadExit(void *pData)
{
    OUTPUT("@CClient4MQTT::__threadExit call");
}

void CClient4MQTT::__threadTimer(void *pData)
{
    OUTPUT("@CClient4MQTT::__threadTimer call");
...
// 发布消息事件实现
void CClient4MQTT::__threadPublish(void *pData)
{
    OUTPUT("@CClient4MQTT::__threadPublish call");
...
// 连接丢失事件实现
void CClient4MQTT::__threadConnList(void *pData)
{
    OUTPUT("@CClient4MQTT::__threadConnList call");
...
// 连接服务端
bool CClient4MQTT::__tryConnecServer()
{
    ...
    {
        OUTPUT("@CClient4MQTT::__tryConnecServer connect server:[%s] failed, rc:[%d]", m_strServerAddress.c_str(), rc);
        MQTTClient_destroy(&m_client);
        return false;
    }

    OUTPUT("@CClient4MQTT::__tryConnecServer connect server:[%s] ok", m_strServerAddress.c_str());
...
// 释放连接
void CClient4MQTT::__freeConnection()
{
...
// 订阅主题
void CClient4MQTT::__subscribing()
{
    for (std::vector<std::string>::const_iterator c_iter = m_vecSubTopics.begin(); c_iter != m_vecSubTopics.end(); ++c_iter)
    {
        ...
        if (rc != MQTTCLIENT_SUCCESS)
        {
            OUTPUT("@CClient4MQTT::__subscribing subscribe topic:[%s] failed, rc:[%d]", c_iter->c_str(), rc);
            continue;
        }

        OUTPUT("@CClient4MQTT::__subscribing subscribe topic:[%s] ok", c_iter->c_str());
    }
}

// 发布消息
bool CClient4MQTT::__publishMsg(const std::string &strTopic, const std::string &strMessage)
{
    ...
    if (rc != MQTTCLIENT_SUCCESS)
    {
        OUTPUT("@CClient4MQTT::__publishMsg publish topic:[%s] msg len:[%d] first100:[%s] failed, rc:[%d]",
               strTopic.c_str(), strMessage.size(), strMessage.substr(0, 100).c_str(), rc);
        return false;
    }

    OUTPUT("@CClient4MQTT::__publishMsg publish topic:[%s] msg len:[%d] first100:[%s] ok",
           strTopic.c_str(), strMessage.size(), strMessage.substr(0, 100).c_str());
    return true;
}

// 来自MQTT的连接断开回调
void CClient4MQTT::__connLostFromMQTT(void *pContext, char *pCause)
{
    CClient4MQTT *pThis = (CClient4MQTT *)pContext;

    OUTPUT("@CClient4MQTT::__connLostFromMQTT connection lost, reason:[%s]", pCause);

    pThis->m_thread.emitEvent("connlost");
}

// 来自MQTT的消息到达回调
int CClient4MQTT::__msgArrvdFromMQTT(void *pContext, char *pTopicName, int nTopicLen, MQTTClient_message *pMessage)
{
    CClient4MQTT *pThis = (CClient4MQTT *)pContext;
    ...
    OUTPUT("@CClient4MQTT::__msgArrvdFromMQTT recv message, topic:[%s] msg len:[%d] first100:[%s]",
           strTopic.c_str(), strMessage.size(), strMessage.substr(0, 100).c_str());
